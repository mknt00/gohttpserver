...
func main() {
	...
	router.HandleFunc("/-/stats", func(w http.ResponseWriter, r *http.Request) {
		...
		items := make([]map[string]interface{}, 0, len(downloadCounts.count))
		for k, v := range downloadCounts.count {
			items = append(items, map[string]interface{}{"path": k, "downloads": v})
		}
		downloadCounts.mutex.RUnlock()
		data, _ := json.Marshal(map[string]interface{}{"downloads": items})
		w.Header().Set("Content-Type", "application/json")
		w.Write(data)
	})
